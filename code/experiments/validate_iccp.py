...
的洞穴微气候：12 个岩溶洞、42 个记录仪（此两项及起止日期取自 Zenodo 记录页元数据，
仓内无法复算），逐时气温与相对湿度，气候区（荒漠 / 山地 / 加利利）与岩性（石灰岩 / 白云岩）
与莫高窟（砾岩—砂岩、干旱区）完全不同。

本脚本实际使用的规模（**可复算**）：8 个有效洞（筛选链条 12 → 9 → 8），
`code/results/iccp_transfer.csv` 的 `n_test` 列求和 = **16,944** 个逐小时配对时次。
（早期文档曾写「15 700 小时」，该数字在仓内无任何落盘来源，已废弃。）
