...
SEGMENTS = [
    ...
    (5, 16, "这是最关键的实证：只用 MSE 训练时，点精度 R² 达到 0.59，"
            "但超阈召回率只有 0.047，九成五以上的超阈事件被漏报。", True),
    ...
    (10, 14, "基线对比：初稿的递归实现 AUC 约等于 0.5，等于随机猜测；本作品在可部署方案中各时效稳定领先。", True),
    ...
    (16, 28, "这段动画演示 2024 年 4 月 16 日的高湿事件：窟外湿度由 13% 升至 81%，"
             "真实窟内湿度将达 89%，而预报曲线明显低估了它——这正是我们改进读出层的动因。", False),   # 动画段
    (17, 15, "业务价值不在平均误差，而在提前量：62% 档平均提前 46.3 小时，可部署基线最高 42.5 小时。", True),
    (18, 15, "四个创新点全部落在算法层——三个模型级、一个系统级，不含应用层包装。", True),
    (19, 14, "我们主动声明能力边界：窟内无公开实测数据，极值捕捉比仅 0.61 到 0.69。", True),
