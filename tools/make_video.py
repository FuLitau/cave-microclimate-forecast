...
# ------------------------------------------------------------------ 主流程
SEGMENTS = [
    # (页号, 秒数, 字幕, 是否淡入淡出)  —— 总长 275 s（4:35），与页数严格一一对应
    ...
    (14, 14, "第二条外部链路换成完全真实的观测：ICCP 十二个岩溶洞穴。把洞口湿度直接当深处湿度，"
             "比永远预测均值还差；算子优于一阶传递函数七比八洞。", True),
    (15, 28, "这段动画演示 2024 年 4 月 16 日的高湿事件：窟外相对湿度由 13% 升至 81%，"
             "系统在预报推进中给出分级预警。", False),   # 动画段
    (16, 15, "业务价值不在平均误差，而在提前量：62% 档平均提前 42.6 小时，初稿方案为零。", True),
    (17, 15, "四个创新点全部落在算法层，核心创新层级为三个模型级、一个系统级。", True),
    (18, 14, "我们主动声明能力边界：窟内无公开实测数据，极值捕捉比仅 0.6 到 0.68。", True),
    (19, 12, "后续将接入真实窟内实测、图版数字化标签，并把点预报升级为概率预报。", True),
    (20, 7, "恳请各位评委批评指正。", True),
]
