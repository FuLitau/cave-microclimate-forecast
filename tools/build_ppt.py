...
para(tf, "A 组（仅 MSE）：点精度 R² = 0.589，但超阈召回率仅 0.047 "
         "→ 漏报 95% 以上的超阈事件。", size=12.5, color=DARK,
     first=True, space_after=3)
para(tf, "B 组（MSE + 阈值加权 twCRPS）：点精度不降反升（R² = 0.652），"
         "超阈 F1 由 0.072 → 0.141，AUC 由 0.862 → 0.901。", size=12.5, bold=True,
...
para(tf, "该场景下窟内真实 RH 在 72 h 内升至 89.0%（触发全部三级阈值）。"
         "演示面板如实显示：状态式线性读出最高只报出 38.4%，62% 档报警一次都未触发。",
     size=12, color=DARK, first=True, space_after=4, line=1.25)
para(tf, "这正是第 12 页读出层修正、以及「用风险分数而非点预报判级」"
         "两条改进的直接动因。", size=12, color=RED, space_after=0,
     line=1.25)
notes(s, "演示案例用真实外场驱动。这里如实展示当前版本在极值场景仍报不出 62% 警报——"
         "我们把它和读出层修正、风险分数判级连起来讲，"
         "形成「发现问题→归因→修正」的完整链条。")
...
rows = [
    ["阈值", "本作品检出率", "平均提前量", "初稿·静态传递检出率"],
    ["62% 业务预警", "35.1%", "46.7 h", "29.8%"],
    ["67% 潮解起始", "22.5%", "23.0 h", "15.0%"],
    ["75% 吸湿突变", "9.1%", "4.0 h", "0.0%"],
]
table(s, rows, 0.62, 5.55, 12.1, 1.45, col_w=[1.3, 1.3, 1.2, 1.8], size=11, head_size=11.5)
notes(s, "46.7 小时的提前量对应现实中的管理动作：当天下午就能决定次日是否限流、"
         "是否安排除湿设备。同时如实说明：75% 档事件极少（测试段仅 11 次起始事件），"
         "检出率只有 9.1%、平均提前量仅 4.0 小时。")
